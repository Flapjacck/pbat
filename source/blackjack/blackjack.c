...
int blackjack_main(int argc, char* argv[])
{
    ...
    // Initialize hands to NULL so clear_hands works properly
    ...
    while (cash > 0 && play_again == 'y') {
        ...
        // Give a brief pause and refresh screen
        printf("Dealing cards...\n");
        printf("Debug: About to call deal_initial_cards\n");
        gfxFlushBuffers();
        gfxSwapBuffers();
        gspWaitForVBlank();
        ...
        printf("Debug: deal_initial_cards completed\n");
        gfxFlushBuffers();
        gfxSwapBuffers();
        gspWaitForVBlank();
    ...
    // Graphics will be cleaned up by main menu
    return 0;
}
